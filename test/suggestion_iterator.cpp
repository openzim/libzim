...
TEST(suggestion_iterator, rangeBased) {
  ...
  zim::Archive::EntryRange<zim::EntryOrder::titleOrder> range = archive.findByTitle("article");
  auto srs = search.testRangeBased(range);
  ASSERT_EQ(srs.size(), 2);

  auto it1 = srs.begin();
  ASSERT_EQ(it1->getTitle(), "article a");
  ASSERT_EQ(it1.getDbData(), "");
