...
void setDataDir(std::string& dataDir)
{
  ...
  if (cDataDir == NULL) {
    ...
    FAIL() << "ZIM_TEST_DATA_DIR is not defined. You must define it to the directory containing test zim files. See https://github.com/openzim/zim-testing-suite .";
  }
  dataDir = cDataDir;
}
...
std::string getDataFilePath(const std::string& filename)
{
  std::string dataDir;
  setDataDir(dataDir);
  return zim::DEFAULTFS::join(dataDir, filename);
}
