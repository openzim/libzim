...
struct TestCacheConfig {
  size_t dirent_cache_size;
  size_t cluster_cache_size;
...
TEST_F(ZimArchive, cacheDontImpactReading)
{
  ...
  for (auto& testfile: getDataFilePath("small.zim")) {
    ...
    for (auto cache_config: cache_configs) {
      auto test_archive = zim::Archive(testfile.path);
      test_archive.set_dirent_cache_max_size(cache_config.dirent_cache_size);
      test_archive.set_dirent_lookup_cache_max_size(cache_config.dirent_lookup_cache_size);
      zim::set_cluster_cache_max_size(cache_config.cluster_cache_size);

      EXPECT_EQ(test_archive.get_dirent_cache_max_size(), cache_config.dirent_cache_size);
      EXPECT_EQ(test_archive.get_dirent_lookup_cache_max_size(), cache_config.dirent_lookup_cache_size);
      EXPECT_EQ(zim::get_cluster_cache_max_size(), cache_config.cluster_cache_size);
...
TEST_F(ZimArchive, cacheClean) {
  for (auto& testfile: getDataFilePath("wikibooks_be_all_nopic_2017-02.zim")) {
    EXPECT_EQ(zim::get_cluster_cache_current_size(), 0); // No clusters in cache
    {
      ...
      EXPECT_GT(zim::get_cluster_cache_current_size(), 0); // No clusters in cache
    }
    EXPECT_EQ(zim::get_cluster_cache_current_size(), 0); // No clusters in cache
  }
}
...
TEST_F(ZimArchive, cacheChange)
{
  ...
  for (auto& testfile: getDataFilePath("wikibooks_be_all_nopic_2017-02.zim", {"noTitleListingV0"})) {
    ...
    zim::set_cluster_cache_max_size(L2_SIZE);
    ...
    EXPECT_LE(zim::get_cluster_cache_current_size(), L2_SIZE);

    // Reduce cache size
    archive.set_dirent_cache_max_size(10);
    zim::set_cluster_cache_max_size(L1_SIZE);

    EXPECT_EQ(archive.get_dirent_cache_current_size(), 10);
    EXPECT_LE(zim::get_cluster_cache_current_size(), L1_SIZE);
...
TEST_F(ZimArchive, MultiZimCache)
{
  ...
  zim::set_cluster_cache_max_size(BIG_LIMIT);
  ...
  EXPECT_LE(zim::get_cluster_cache_current_size(), BIG_LIMIT);
  zim::set_cluster_cache_max_size(SMALL_LIMIT);
  EXPECT_LE(zim::get_cluster_cache_current_size(), SMALL_LIMIT);

  // Opening an archive should increase the cluster cache
  zim::set_cluster_cache_max_size(BIG_LIMIT);
  auto current_limit = zim::get_cluster_cache_current_size();
  {
    ...
    EXPECT_GT(zim::get_cluster_cache_current_size(), current_limit);
    current_limit = zim::get_cluster_cache_current_size();
  }
  // Destroying an archive should decrease the cluster cache
  EXPECT_LT(zim::get_cluster_cache_current_size(), current_limit);
  ...
  while (!archives.empty()) {
    archives.pop_back();
    EXPECT_LE(zim::get_cluster_cache_current_size(), current_limit);
    current_limit = zim::get_cluster_cache_current_size();
  }
  EXPECT_EQ(zim::get_cluster_cache_current_size(), 0);
}
