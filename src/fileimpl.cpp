...
private: // data
  ...
  FileImpl::~FileImpl() {
    ...
    clusterCache.drop_all([=](const cluster_index_type key) {return true;});
  }
...
bool checkTitleListing(const IndirectDirentAccessor& accessor, entry_index_type totalCount) {
  ...
  void FileImpl::set_cluster_cache_max_size(size_t size_in_b) {
    clusterCache.set_max_size(size_in_b);
  }
