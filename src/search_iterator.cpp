...
{
    if (it.mp_searchIterator) mp_searchIterator = std::unique_ptr<SearchIterator>(new SearchIterator(*it.mp_searchIterator));
...
SuggestionIterator & SuggestionIterator::operator=(const SuggestionIterator& it) {
    ...
    if (it.mp_rangeIterator) mp_rangeIterator.reset(new RangeIterator(*it.mp_rangeIterator));

    mp_searchIterator.reset();
    if (it.mp_searchIterator) mp_searchIterator.reset(new SearchIterator(*it.mp_searchIterator));
