...
{
  return m_file->getBlob(m_dirent->getClusterNumber(), m_dirent->getBlobNumber(), offset_t(offset));
}

Blob Item::getData(offset_type offset, size_type size) const
{
  return m_file->getBlob(m_dirent->getClusterNumber(), m_dirent->getBlobNumber(), offset_t(offset), zsize_t(size));
}
